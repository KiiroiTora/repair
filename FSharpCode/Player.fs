...
type PlayerFS() as this =
    ...
    let free_hand() = 
        ...
        | _ -> None
    member val client_state = ({
        mouse_pos = this.GlobalPosition;
        is_charge_pressed = false
        is_charge_just_released = false
        is_run_pressed = false
        
    }) with get, set
    member this.mouse_distance() = (this.client_state.mouse_pos - this.GlobalPosition).Length()
    member this.controller_dir() =  if ((this.client_state.mouse_pos - this.GlobalPosition).Length() > 1.0f) 
                                          then (this.client_state.mouse_pos - this.GlobalPosition).Normalized() 
                                          else Vector2.Zero
    ...
    override this._Process (delta) =
        ...
        let is_charging = this.client_state.is_charge_pressed && (has_axe_l || has_axe_r)
        ...
        let is_running = this.client_state.is_run_pressed
        ...
        if this.client_state.is_charge_just_released then do this.throw()
